...
namespace MovieStoreExamen.Controllers
{
    ...
    public class UsersController : ApplicationController
    {
        ...
        public IActionResult Index(string userName, string name, string email, int? pageNumber)
        {
            ...
            foreach (var user in users)
            {
                userViewModels.Add(new ApplicationUserViewModel
                {
                    ...
                    Administrator = _context.UserRoles.Where(ur => ur.UserId == user.Id && ur.RoleId == "Administrator").Count() > 0,
                    Customer = _context.UserRoles.Where(ur => ur.UserId == user.Id && ur.RoleId == "Customer").Count() > 0,
                    Worker = _context.UserRoles.Where(ur => ur.UserId == user.Id && ur.RoleId == "Worker").Count() > 0
                });
        ...
        public async Task<ActionResult> Roles([Bind("Id, UserName, FirstName, LastName, Administrator, Customer, Worker")] ApplicationUserViewModel model)
        {
            ...
            if (model.Administrator) _context.Add(new IdentityUserRole<string> { RoleId = "Administrator", UserId = model.Id });
            if (model.Customer) _context.Add(new IdentityUserRole<string> { RoleId = "Customer", UserId = model.Id });
            if (model.Worker) _context.Add(new IdentityUserRole<string> { RoleId = "Worker", UserId = model.Id });
            await _context.SaveChangesAsync();
            ; return RedirectToAction("Index");
        }
    }
